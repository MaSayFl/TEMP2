...
if [ -f ${PREFIX}.pair.uniq.split.fastq ] && [ -f ${PREFIX}.pair.uniq.split.bed ];then
	...
	sambamba view -t ${CPU} -h -F "proper_pair and not supplementary" ${BAM} | parallel -j 20 --pipe 'grep "SA:Z:"' | cat ${PREFIX}.tmp.header - | samtools view -@ ${CPU} -bhS - | samtools sort -@ ${CPU} -o ${PREFIX}.pair.uniq.split.temp.bam -
 	sambamba view -t ${CPU} -h ${PREFIX}.pair.uniq.split.temp.bam | parallel -j ${CPU} --pipe 'gawk -v  uniq_ratio=${UNIQ_RATIO} '\''BEGIN{FS=OFS="\t"} {for(i=12;i<=100;i++){if($i~/AS:i:/){as=substr($i,6)};if($i~/XS:i:/){xs=substr($i,6);break}};if(xs/1<as*uniq_ratio){print $0}}'\'' -' | parallel --pipe gawk '(and($2,16)==0 && $6~/^[0-9]+S/) || (and($2,16)==16 && $6~/S$/)' - | cat ${PREFIX}.tmp.header - | samtools view -@ ${CPU} -bhS - | samtools sort -@ ${CPU} -o ${PREFIX}.pair.uniq.split.bam -
